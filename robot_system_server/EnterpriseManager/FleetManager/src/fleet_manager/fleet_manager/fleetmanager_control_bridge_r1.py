...
class RobotStatusPublisher(Node):
    ...
    def __init__(self, shared_state):
        ...
        self.publisher = self.create_publisher(RobotStatus, 'robot1/RobotStatus', 10)
    ...
    def status_callback(self):
        ...
        if self.battery_time > 100:
            if 100> self.battery_status > 0:  # 배터리가 0 이하로 내려가지 않도록
...
class JobAllocatedServer(Node):
    ...
    def __init__(self, shared_state):
        ...
        # # 작업 받는 서버
        self.joballocate_server = self.create_subscription(JobAllocated, 'robot1/JobAllocatedRes', self.allocatd_job, 10, callback_group=self.callback_group)
        # 주행 상태 받는 서버
        ...
        # 작업 완료 클라이언트
        self.jobcomplete_client = self.create_publisher(JobCompleteReq, 'robot1/JobCompleteReq', 10, callback_group=self.callback_group)
        # 픽업 동작 요청 클라이언트
    ...
    def yolo_callback(self, msg):
        ...
        #     if self.detected_walking == 1:
        #         self.get_logger().info("Walking detected, canceling task...")
        #         self.nav.cancelTask()  # 현재 주행 작업 취소
        #     elif self.detected_walking == 0 and self.last_goal_pose is not None:
